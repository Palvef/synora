...
UPSTREAM=${SYNORA_UPSTREAM:-"rsync://master.bioconductor.org"}
VERSIONS=("3.5")
RELEASE_VERSION="3.5"
...
mkdir -p ${SYNORA_STORAGE}/packages

for version in ${VERSIONS[@]}; do
	upstream=${UPSTREAM}/${version}
	dest=${SYNORA_STORAGE}/packages/${version}
...
ln -sfT ${RELEASE_VERSION} ${SYNORA_STORAGE}/packages/release
