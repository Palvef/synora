...
UPSTREAM_BASE=${SYNORA_UPSTREAM:-"https://llvm.org/git"}
...
for repo in ${repos[@]}; do
	if [[ ! -d "$SYNORA_STORAGE/${repo}.git" ]]; then
		...
		repo_init "${UPSTREAM_BASE}/${repo}" "$SYNORA_STORAGE/${repo}.git"
	fi
	repo_update "${UPSTREAM_BASE}/${repo}" "$SYNORA_STORAGE/${repo}.git"
done
