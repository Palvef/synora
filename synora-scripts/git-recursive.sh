...
path=$(basename -- "$SYNORA_STORAGE")
path_no_git=${path%%.git}
mirror_url=$(echo "$SYNORA_STORAGE" | sed "s#$WORKING_DIR_BASE#$MIRROR_BASE_URL#")
script_append "mkdir -p ${path_no_git@Q}"
script_append "git clone ${mirror_url@Q} ${path_no_git@Q}"
script_append "pushd ${path_no_git@Q}"
git_sync_recursive "$SYNORA_UPSTREAM" "$SYNORA_STORAGE"
