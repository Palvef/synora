...
OFFICIAL="${TUNASYNC_UPSTREAM_URL:-https://static.rust-lang.org/}"
BASE_URL=${MIRROR_BASE_URL:-${TUNASYNC_MIRROR_BASE_URL:-"https://mirrors.nyist.edu.cn/rustup"}}
GC=${RUSTUP_GC:-"30"}

if ! command -v python3 >/dev/null 2>&1 || ! command -v curl >/dev/null 2>&1; then
  echo "python3/curl missing; this job should use tunathu/tunasync-scripts" >&2
...
"$MIRROR_BIN" -u "${BASE_URL}" -U "http://127.0.0.1:${PORT}/" -m "${TUNASYNC_WORKING_DIR}" --gc "${GC}"
